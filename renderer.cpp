...
double Renderer::triangleSignedArea(double ax, double ay, double bx, double by, double cx, double cy) const {
  return ((bx - ax) * (cy - ay) - (by - ay) * (cx - ax)) / 2;
}

bool Renderer::isInsideTriangle(double x, double y, const Triangle& triangle) const {
...
void Renderer::rasterizeTriangle(const Triangle& triangle, Screen& screen) {
  ...
  for (uint32_t x = minX; x < maxX; ++x) {
    for (uint32_t y = minY; y < maxY; ++y) {
      if (isInsideTriangle(x, y, triangle)) {
        screen.setPixel(x, y, sf::Color(255, 255, 255));
      }
    }
  }
}

void Renderer::render(World& world, Screen& screen) {
